...
PERIODICOS_LIST_FILE = Path("scripts/periodicos_unicos.txt")
...
def save_state(i):
    STATE_FILE.write_text(json.dumps({"last_index": i}, indent=2), encoding="utf-8")


def load_urls():
    return [
        l.strip()
        for l in PERIODICOS_LIST_FILE.read_text(encoding="utf-8").splitlines()
        if l.strip()
    ]
...
def process_periodico(url, idx, total):
    ...
    for child in children:
        ...
        path = METADATA_DIR / f"{norm}_{cid}.json"
        if path.exists():
            continue
        ...
        new_count += 1
        print(f"   → {path.name} (year={year if year else '??'})")

    if new_count == 0:
        print("   (sin nuevos items)")
    else:
        print(f"   ✔ {new_count} metadata generados")
...
def main():
    ...
    print(f"✔ metadata completada [{end-start}] periódico(s)")
